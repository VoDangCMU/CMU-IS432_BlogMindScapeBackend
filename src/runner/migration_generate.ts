...
const dataSourcePath = path.resolve(__dirname, "..", "database", "DataSource.ts");
...
function run() {
	...
	exec(`yarn typeorm migration:generate ${migrationFilePath} -d ${dataSourcePath}`, (error, stdout, stderr) => {
		if (error) {
			logger.error(error);
			return;
		}
